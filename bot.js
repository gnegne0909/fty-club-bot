...
// Check permission (toujours true, géré côté panel)
...
// Modération depuis le panel (ban/kick/warn)
app.post('/api/moderate', verifyApiKey, async (req, res) => {
    ...
    try {
        ...
        const titles = { ban:'🔨 Vous avez été banni', kick:'👢 Vous avez été expulsé', warn:'⚠️ Avertissement' };
        const embed = new EmbedBuilder()
            .setColor(colors[action] || '#888')
            .setTitle(titles[action] || '📋 Action Staff')
