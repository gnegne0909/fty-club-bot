...
async function sendToPanel(action, data) {
    try {
        console.log(`[BOT→PANEL] ${action}`);
        
        const response = await axios.post(`${PANEL_URL}/api/bot`, {
            ...
            headers: {
                ...
                'User-Agent': 'FTY-Bot/3.0'
            }
        });
        
        console.log(`[BOT→PANEL] ✅ ${action} OK`);
        return response.data;
    } catch (error) {
        console.error(`[BOT→PANEL] ❌ ${action} - ${error.message}`);
        return null;
    }
}
