...
const client = new Client({
    ...
    // Timeout WebSocket explicite pour éviter les silences infinis sur Render
...
const SLASH_COMMANDS = [
    ...
    {
        name: 'maintenance',
        description: '🔧 Activer/désactiver le mode maintenance du site (Xywez uniquement)',
        default_member_permissions: '0',
        dm_permission: false,
        options: [
            { type: 5, name: 'activer', description: 'true = activer, false = désactiver', required: true },
            { type: 3, name: 'message', description: 'Message de maintenance (optionnel)', required: false }
        ]
    },
...
client.once('clientReady', async () => {
    ...
    console.log('');
    console.log('╔══════════════════════════════════════════════════════╗');
    console.log(`║  ✅ BOT CONNECTÉ: ${client.user.tag.padEnd(33)}║`);
    console.log(`║  📡 Guild: ${botStatus.guilds} serveur(s) | ${botStatus.members} membres${' '.repeat(Math.max(0, 18 - String(botStatus.guilds).length - String(botStatus.members).length))}║`);
    console.log('╚══════════════════════════════════════════════════════╝');
...
client.on('interactionCreate', async interaction => {
    ...
    try {
        ...
        // ── /maintenance ───────────────────────────────────────
            ...
            // Réservé UNIQUEMENT à Xywez
            if (user.id !== SUPER_ADMIN_DISCORD_ID) {
                return await interaction.editReply({
                    embeds: [new EmbedBuilder()
                        .setTitle('❌ Accès Refusé')
                        .setDescription('Cette commande est **réservée à Xywez uniquement**.')
                        .setColor(0xef4444)
                        .setTimestamp()
                    ]
                });
            ...
            const message = interaction.options.getString('message') || (activer
                ? 'Le site est en maintenance. Revenez bientôt !'
                : null);

            try {
                const res = await axios.post(`${PANEL_URL}/api/bot/maintenance`, {
                ...
                const data = res.data;
                if (data.success) {
                    addBotLog(`${activer ? '⚙️ Maintenance ACTIVÉE' : '✅ Maintenance DÉSACTIVÉE'} par Xywez via Discord`, activer ? 'warn' : 'success');

                    await interaction.editReply({
                        embeds: [new EmbedBuilder()
                            .setTitle(activer ? '⚙️ Maintenance Activée' : '✅ Maintenance Désactivée')
                            .setDescription(activer
                                ? `Le site est maintenant **en maintenance**.\nMessage : *${message}*\n\nUtilise \`/maintenance activer:False\` pour le remettre en ligne.`
                                : '✅ Le site est maintenant **en ligne** et accessible à tous !'
                            )
                            .setColor(activer ? 0xf59e0b : 0x22c55e)
                            .setFooter({ text: 'FTY Club Pro | Maintenance System' })
                            .setTimestamp()
                        ]
                    });

                    // Log dans le salon logs si configuré
                    const cfg = readConfig();
                    if (cfg.channels?.logs) {
                        await sendToChannel(cfg.channels.logs, {
                            title: activer ? '⚙️ Site en Maintenance' : '✅ Site Remis en Ligne',
                            description: `**Action :** ${activer ? 'Maintenance activée' : 'Maintenance désactivée'}\n**Par :** Xywez\n${message ? `**Message :** ${message}` : ''}`,
                            color: activer ? 0xf59e0b : 0x22c55e,
                            timestamp: new Date().toISOString(),
                            footer: { text: 'FTY Club Pro | Maintenance' }
                        });
                    }
                } else {
                    await interaction.editReply({ content: `❌ Erreur : ${data.error || 'Réponse invalide du panel'}` });
                }
            } catch(e) {
                addBotLog(`❌ /maintenance error: ${e.message}`, 'error');
                await interaction.editReply({ content: `❌ Erreur de connexion au panel : ${e.message}` });
            }
        }
...
client.on('interactionCreate', async interaction => {
    ...
    if (customId === 'accept_reglement') {
        try {
            ...
            await interaction.reply({ content: '✅ **Règlement accepté !** Tu as maintenant accès au serveur. Bienvenue sur **FTY Club Pro** ! 🎉', flags: MessageFlags.Ephemeral});
            addBotLog(`📜 Règlement accepté par ${user.tag}`, 'discord');
        } catch(e) {
            try { await interaction.reply({ content: '❌ Erreur lors de l\'acceptation.', flags: MessageFlags.Ephemeral}); } catch(_) {}
        }
    }
    ...
    // 🎫 Bouton créer ticket depuis le salon
        ...
        if (existing) return await interaction.reply({ content: `❌ Tu as déjà un ticket ouvert (\`${existing.id}\`). Vérifie tes DMs.`, flags: MessageFlags.Ephemeral });
        const ticketId = `t_${Date.now()}`;
        ...
        if (!dmOk) { delete tickets[ticketId]; writeTickets(tickets); return await interaction.reply({ content: '❌ Tes DMs sont fermés. Active-les dans Paramètres → Confidentialité.', flags: MessageFlags.Ephemeral}); }
        await sendToPanel('newTicket', newTicket);
        ...
        await interaction.reply({ content: `✅ Ticket \`${ticketId}\` ouvert ! Vérifie tes **messages privés** 📨`, flags: MessageFlags.Ephemeral });
    }
});
...
app.post('/api/bot', verifyApiKey, async (req, res) => {
    ...
    try {
        if (action === 'maintenance') {
            ...
            botStatus.panelConnected = true;
        } else if (action === 'getUserDiscordId') {
            // Cherche un membre Discord par username panel dans le serveur
            const username = data?.username;
            if (!username) return res.json({ success: false, error: 'username requis' });
            // On renvoie null si inconnu — le panel stocke le discordId dans la DB
            return res.json({ success: true, discordId: null, note: 'Utilise le discordId stocké dans la DB panel.' });
        }
        res.json({ success: true });
...
// GET / et /health — Render Health Check (garde le service en vie)
...
// ============================================================
// ===  SYSTÈME MATCH — VOC TEMPORAIRE + RÔLES DISCORD     ===
// ============================================================

// Map des voix/rôles match en cours (matchId → {vocId, hoteRoleId, matchRoleId})
const activeMatchVoices = {};

// Créer la voix temporaire du match + rôle hôte
app.post('/api/create-match-voice', verifyApiKey, async (req, res) => {
    const { matchId, matchToken, adversaire, date, competition, hoteUsername } = req.body;
    ...
    try {
        ...
        // ── 1. Trouver ou créer une catégorie "MATCHS" ──
        let matchCat = guild.channels.cache.find(c => c.type === ChannelType.GuildCategory && c.name.toLowerCase().includes('match'));
        if (!matchCat) {
            matchCat = await guild.channels.create({
                name: '⚽ MATCHS',
                type: ChannelType.GuildCategory,
                reason: 'Catégorie auto-créée pour les matchs FTY'
            }).catch(() => null);
        }

        // ── 2. Créer le rôle temporaire HÔTE du match ──
        const hoteRole = await guild.roles.create({
            name: `🎙️ Hôte Match vs ${adversaire}`.substring(0, 100),
            color: 0x9333ea,
            mentionable: false,
            reason: `Rôle hôte match ${matchToken}`
        }).catch(() => null);

        // ── 3. Créer le rôle temporaire MATCH (convoqués) ──
        const matchRole = await guild.roles.create({
            name: `⚽ Convoqué vs ${adversaire}`.substring(0, 100),
            color: 0x22c55e,
            mentionable: true,
            reason: `Rôle match ${matchToken}`
        }).catch(() => null);

        // ── 4. Créer le salon vocal temporaire ──
        const permissionOverwrites = [
            // @everyone ne voit pas
            { id: guild.roles.everyone.id, deny: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.Connect] },
            // Rôle matchRole peut voir + se connecter
            ...(matchRole ? [{ id: matchRole.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.Connect, PermissionFlagsBits.Speak] }] : []),
            // Rôle hôte peut TOUT + mute + virer les membres
            ...(hoteRole ? [{ id: hoteRole.id, allow: [
            ...
            // Admins/mods aussi
            ...(cfg.roles?.admin ? [{ id: cfg.roles.admin, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.Connect, PermissionFlagsBits.MuteMembers, PermissionFlagsBits.MoveMembers] }] : []),
            ...(cfg.roles?.moderateur ? [{ id: cfg.roles.moderateur, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.Connect, PermissionFlagsBits.MuteMembers, PermissionFlagsBits.MoveMembers] }] : [])
        ];

        const vocChannel = await guild.channels.create({
            name: `⚽・vs ${adversaire}`.substring(0, 100),
            type: ChannelType.GuildVoice,
            parent: matchCat?.id,
            permissionOverwrites,
            reason: `Match vocal ${matchToken}`
        }).catch(() => null);

        // ── 5. Stocker les infos en mémoire ──
        activeMatchVoices[matchId] = {
            vocId: vocChannel?.id,
            hoteRoleId: hoteRole?.id,
            matchRoleId: matchRole?.id,
            adversaire,
            createdAt: Date.now()
        };

        // ── 6. Notifier le panel des IDs créés ──
        try {
            await axios.post(`${PANEL_URL}/api/match/voice-created`, {
                apiKey: PANEL_API_KEY,
                matchId,
                vocChannelId: vocChannel?.id,
                hoteRoleId: hoteRole?.id,
                matchRoleId: matchRole?.id
            }, { timeout: 8000 });
        } catch(e) {}

        // ── 7. Annoncer dans le salon match-annonce ──
        if (cfg.channels?.matchAnnonce) {
            await sendToChannel(cfg.channels.matchAnnonce, {
                title: `⚽ Nouveau Match Annoncé !`,
                description: `**FTY vs ${adversaire}**\n📅 ${date}\n🏆 ${competition}\n\n${vocChannel ? `🎙️ Voix du match : <#${vocChannel.id}>` : ''}`,
                color: 0x9333ea,
                timestamp: new Date().toISOString(),
                footer: { text: 'FTY Club Pro | Matchs' }
            });
        }

        addBotLog(`⚽ Match voice créée: vs ${adversaire} | Voc: ${vocChannel?.name} | Hôte role: ${hoteRole?.name}`, 'success');
        res.json({ success: true, vocChannelId: vocChannel?.id, hoteRoleId: hoteRole?.id, matchRoleId: matchRole?.id });

    } catch(e) {
        addBotLog(`❌ create-match-voice: ${e.message}`, 'error');
        res.json({ success: false, error: e.message });
    }
});

// Attribuer les rôles match aux convoqués + hôte
app.post('/api/match-assign-roles', verifyApiKey, async (req, res) => {
    const { matchId, convoqués, hoteUsername, adversaire, date } = req.body;
    if (!matchId || !convoqués) return res.status(400).json({ error: 'matchId + convoqués requis' });

    try {
        const guild = client.guilds.cache.get(GUILD_ID);
        if (!guild) return res.json({ success: false, error: 'Serveur introuvable' });

        const voiceData = activeMatchVoices[matchId];
        if (!voiceData) return res.json({ success: false, error: 'Voix du match introuvable (non créée ?)' });

        let rolesAssigned = 0;

        // ── Attribuer rôle "Convoqué" aux joueurs ──
        for (const c of convoqués) {
            if (!c.discordId) continue;
            try {
                const member = await guild.members.fetch(c.discordId).catch(() => null);
                if (member && voiceData.matchRoleId) {
                    await member.roles.add(voiceData.matchRoleId).catch(() => {});
                    rolesAssigned++;
                    addBotLog(`✅ Rôle match → ${c.username}`, 'discord');
                }
            } catch(e) {}
        }

        // ── Attribuer rôle "Hôte" au responsable du match ──
        if (voiceData.hoteRoleId) {
            const hoteDiscordId = req.body.hoteDiscordId;
            if (hoteDiscordId) {
                try {
                    const hoteMember = await guild.members.fetch(hoteDiscordId).catch(() => null);
                    if (hoteMember) {
                        await hoteMember.roles.add(voiceData.hoteRoleId).catch(() => {});
                        addBotLog(`🎙️ Rôle hôte → ${hoteUsername} (${hoteDiscordId})`, 'success');
                    }
                } catch(e) {}
            }
        }

        // Annoncer dans le salon match
        if (activeMatchVoices[matchId]?.vocId) {
            const cfg = readConfig();
            if (cfg.channels?.matchAnnonce) {
                const mentions = convoqués.filter(c => c.discordId).map(c => `<@${c.discordId}>`).join(' ');
                await sendToChannel(cfg.channels.matchAnnonce, {
                    title: `📋 Composition Validée — FTY vs ${adversaire}`,
                    description: `Les joueurs convoqués ont reçu leur rôle Discord et ont accès à la voix du match.\n\n${mentions}\n\n🎙️ Voix : <#${voiceData.vocId}>`,
                    color: 0x22c55e,
                    timestamp: new Date().toISOString(),
                    footer: { text: 'FTY Club Pro | Matchs' }
        ...
        addBotLog(`⚽ Rôles match assignés: ${rolesAssigned}/${convoqués.length} convoqués`, 'success');
        res.json({ success: true, rolesAssigned });

    } catch(e) {
        addBotLog(`❌ match-assign-roles: ${e.message}`, 'error');
        res.json({ success: false, error: e.message });
    }
});

// Remplacement d'un joueur : retirer rôle sortant, attribuer au entrant
app.post('/api/match-replace', verifyApiKey, async (req, res) => {
    const { matchId, sortant, sortantDiscordId, entrant, entrantDiscordId } = req.body;
    try {
        const guild = client.guilds.cache.get(GUILD_ID);
        if (!guild) return res.json({ success: false, error: 'Serveur introuvable' });

        const voiceData = activeMatchVoices[matchId];
        if (!voiceData?.matchRoleId) return res.json({ success: false, error: 'Données du match introuvables' });

        // Retirer rôle du sortant
        if (sortantDiscordId) {
            const sortantMember = await guild.members.fetch(sortantDiscordId).catch(() => null);
            if (sortantMember) await sortantMember.roles.remove(voiceData.matchRoleId).catch(() => {});
        }
        // Attribuer rôle à l'entrant
        if (entrantDiscordId) {
            const entrantMember = await guild.members.fetch(entrantDiscordId).catch(() => null);
            if (entrantMember) await entrantMember.roles.add(voiceData.matchRoleId).catch(() => {});
        }

        addBotLog(`🔄 Remplacement match: ${sortant} → ${entrant}`, 'discord');
        res.json({ success: true });
    } catch(e) {
        addBotLog(`❌ match-replace: ${e.message}`, 'error');
        res.json({ success: false, error: e.message });
    }
});

// Supprimer la voix + rôles du match (annulation)
app.post('/api/delete-match-voice', verifyApiKey, async (req, res) => {
    ...
    try {
        ...
        const voiceData = activeMatchVoices[matchId];
        if (!voiceData) return res.json({ success: false, error: 'Aucune donnée pour ce match' });

        // Supprimer le salon vocal
        if (voiceData.vocId) {
            const voc = guild.channels.cache.get(voiceData.vocId);
            if (voc) await voc.delete('Match annulé').catch(() => {});
        }
        // Supprimer les rôles temporaires
        if (voiceData.hoteRoleId) {
            const r = guild.roles.cache.get(voiceData.hoteRoleId);
            if (r) await r.delete('Match annulé').catch(() => {});
        }
        if (voiceData.matchRoleId) {
            const r = guild.roles.cache.get(voiceData.matchRoleId);
            if (r) await r.delete('Match annulé').catch(() => {});
        }

        delete activeMatchVoices[matchId];
        addBotLog(`🗑️ Match vocal + rôles supprimés: ${matchId}`, 'success');
        res.json({ success: true });
    } catch(e) {
        addBotLog(`❌ delete-match-voice: ${e.message}`, 'error');
        res.json({ success: false, error: e.message });
    }
});
...
// ============================================================
// ===           CONNEXION DISCORD (avec diagnostic)       ===
// ============================================================
// ── Timeout de sécurité : si pas de ready en 120s, log l'erreur ──
const loginTimeout = setTimeout(() => {
    console.error('');
    console.error('╔══════════════════════════════════════════════════════╗');
    console.error('║  ⚠️  TIMEOUT: Bot non connecté après 120 secondes  ║');
    console.error('║  → Discord WebSocket lent ou inaccessible (Render)   ║');
    console.error('║  → Discord.js continue de retry automatiquement...  ║');
    console.error('║  → Si le bot ne démarre jamais : régénère le token  ║');
    console.error('║    discord.com/developers → Bot → Reset Token        ║');
    console.error('╚══════════════════════════════════════════════════════╝');
    addBotLog('⚠️ Timeout connexion Discord (120s) — retry en cours...', 'warn');
    // On continue quand même, Discord.js va retry automatiquement
}, 120000);

// Annuler le timeout dès que le bot est connecté
client.once('clientReady', () => clearTimeout(loginTimeout));

console.log('🔄 Connexion Discord en cours...');
console.log('  Token présent:', !!DISCORD_BOT_TOKEN, '| Longueur:', DISCORD_BOT_TOKEN ? DISCORD_BOT_TOKEN.length : 0);
console.log('  Gateway: wss://gateway.discord.gg | Render peut être lent (60-120s)...');

client.login(DISCORD_BOT_TOKEN).then(() => {
    console.log('✅ client.login() résolu — en attente de ready...');
    console.log('  WS Status:', client.ws.status, '| Ping:', client.ws.ping + 'ms');
}).catch(err => {
    clearTimeout(loginTimeout);
    console.error('');
    console.error('╔══════════════════════════════════════════════════════╗');
    console.error('║        ❌  ERREUR CONNEXION DISCORD BOT             ║');
    console.error('╠══════════════════════════════════════════════════════╣');
    console.error('  Code erreur :', err.code || 'N/A');
    console.error('  Message     :', err.message);
    console.error('╠══════════════════════════════════════════════════════╣');
    console.error('║  Solutions :                                         ║');
    console.error('║  1. Token invalide → Reset sur discord.com/dev      ║');
    console.error('║  2. Intents non activés (voir discord.com/dev)      ║');
    console.error('║  3. Bot supprimé ou révoqué                         ║');
    console.error('╚══════════════════════════════════════════════════════╝');
    addBotLog(`❌ Login échoué: ${err.message}`, 'error');
    setTimeout(() => process.exit(1), 3000);
});

// Reconnexion auto
client.on('shardDisconnect', (event) => {
    console.warn(`[BOT] Shard déconnecté (code: ${event?.code})`);
    addBotLog(`⚠️ Déconnecté (code: ${event?.code})`, 'warn');
});

client.on('shardError', (error) => {
    console.error('[BOT] Erreur WebSocket shard:', error.message);
    addBotLog(`❌ Erreur WebSocket: ${error.message}`, 'error');
});

client.on('shardReady', (id) => {
    console.log(`[BOT] Shard ${id} prêt`);
});
